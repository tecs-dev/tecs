...
if not _isBenchChild then
    print("evolved.lua version:", evo.__VERSION)
end
...
local cases = {
    -- Spawn (table storage)
    {
        name = "spawn (no hooks)",
        scenario = "spawnLoop1",
        parameters = { count = { 2000 }, defer = { false, true } },
    },
    {
        name = "spawn 2 components",
        scenario = "spawnLoop2",
        parameters = { count = { 2000 }, defer = { false, true } },
    },
    {
        name = "spawn 4 components",
        scenario = "spawnLoop4",
        parameters = { count = { 2000, 10000 }, defer = { false, true } },
    },
    ...
    {
        name = "bundle/prefab spawn 4 components",
        scenario = "bundleSpawn4",
        parameters = { count = { 10000 }, defer = { false, true } },
    },
    -- Spawn (FFI storage)
    {
        name = "spawn 2 FFI components",
        scenario = "spawnLoop2FFI",
        parameters = { count = { 2000 }, defer = { false, true } },
    },
    {
        name = "batch spawn 4 FFI components",
        scenario = "batchSpawn4FFI",
        parameters = { count = { 10000 }, defer = { false, true } },
    },
    -- Reference: evolved uses split scalar columns instead of one typed struct per entity.
    {
        name = "batch spawn 4 FFI (evolved SoA)",
        scenario = "batchSpawn4FFISoA",
        parameters = { count = { 10000 }, defer = { false, true } },
    },
    -- Despawn
    {
    ...
    -- Mutation
    {
        name = "add component",
        scenario = "addComp",
        parameters = { count = { 1000 }, defer = { false, true } },
    },
    {
    ...
    {
        name = "value-only update",
        scenario = "valueUpdate",
        parameters = { count = { 1000 }, defer = { false, true } },
    },
    {
        name = "value-only update (onReplace)",
        scenario = "valueUpdateHook",
        parameters = { count = { 1000 }, defer = { false, true } },
    },
    ...
    {
        name = "create_with_components (populated)",
        scenario = "createWithComponents",
        parameters = { count = { 100 } },
    },
    {
        name = "batch op (populated)",
        scenario = "batchOp",
        parameters = {
            count = { 1000 },
...
local function fillFFIPositionVelocity(arch, startRow, lastRow)
    local ps = arch:getMut(FFIPosition)
    local vs = arch:getMut(FFIVelocity)
    for i = startRow, lastRow do
        ps[i].x = i
        ps[i].y = i
        vs[i].vx = 1
...
local function populateQueryBuildTecsWorld(world, count)
    for arch = 1, 20 do
        ...
        if arch % 2 == 0 then
            components[#components + 1] = Velocity
        end
        if arch % 3 == 0 then
            components[#components + 1] = Health
        end
        if arch % 4 == 0 then
            components[#components + 1] = Damage
        end
        if arch % 5 == 0 then
            components[#components + 1] = Alive
        end
        if arch % 6 == 0 then
            components[#components + 1] = Aggro
        end
...
local function setupBatchBenchWorld(case)
    ...
    world:batchSpawn(n * 4, { Health, BenchName, Aggro }, function(arch, startRow, lastRow)
        local hs = arch:getMut(Health)
        local ns = arch:getMut(BenchName)
        for i = startRow, lastRow do
            hs[i] = Health(100)
            ns[i] = BenchName("bg")
        end
...
tecsScenarios.spawnLoop1 = {
    setup = function()
        return freshTecsWorld()
    end,
    run = function(world, case)
        if case.params.defer then
            world:defer()
        end
    ...
    run = function(world, case)
        world:batchSpawn(
            ...
            function(arch, startRow, lastRow)
                local ps = arch:getMut(FFIPosition)
                local vs = arch:getMut(FFIVelocity)
                local hs = arch:getMut(FFIHealth)
                local ds = arch:getMut(FFIDamage)
                for i = startRow, lastRow do
                    ps[i].x = i
                    ps[i].y = i
                    vs[i].vx = 1
...
tecsScenarios.sysUpdateFFI = {
    ...
    run = function(state)
        for archetype, len in state.query:iter() do
            local ps = archetype:getMut(FFIPosition)
            local vs = archetype:getMut(FFIVelocity)
            for i = 1, len do
                local p = ps[i]
                local v = vs[i]
                p.x = p.x + v.vx
...
local ArchTagsE = {}
for i = 1, 20 do
    ArchTagsE[i] = evo.builder():tag():build()
end
...
evo.set(ChainBE, evo.ON_INSERT, function(e)
    evo.set(e, ChainCE)
end)
evo.set(ChainAE, evo.ON_INSERT, function(e)
    evo.set(e, ChainBE)
end)
...
local PrefabPosE = evo.builder()
    :duplicate(function()
        return { x = 0, y = 0 }
    end)
    :build()
local PrefabVelE = evo.builder()
    :duplicate(function()
        return { vx = 1, vy = 1 }
    end)
    :build()
local PrefabHealthE = evo.builder()
    :duplicate(function()
        return { hp = 100 }
    end)
    :build()
local PrefabDamageE = evo.builder()
    :duplicate(function()
        return { dmg = 10 }
    end)
    :build()
...
local function ffiDoubleFragment()
    return evo.builder():default(0):realloc(FFI_DOUBLE_REALLOC):compmove(FFI_DOUBLE_COMPMOVE):build()
end
...
local function evoPopulateQueryBuild(count)
    for arch = 1, 20 do
        ...
        if arch % 2 == 0 then
            fragments[VelocityE] = true
        end
        if arch % 3 == 0 then
            fragments[HealthE] = true
        end
        if arch % 4 == 0 then
            fragments[DamageE] = true
        end
        if arch % 5 == 0 then
            fragments[AliveE] = true
        end
        if arch % 6 == 0 then
            fragments[AggroE] = true
        end
...
local function evoPopulateBatchBench(n)
    ...
    evo.multi_spawn_nr(n, {
        [BatchPositionXE] = true,
        [BatchPositionYE] = true,
        [BatchVelocityXE] = true,
        [BatchVelocityYE] = true,
        [AliveE] = true,
    }, function(chunk, b, e)
...
evolvedScenarios.batchSpawn2 = {
    ...
    run = function(_state, case)
        evo.multi_spawn_nr(case.params.count, { [PositionE] = true, [VelocityE] = true }, function(chunk, b, e)
...
evolvedScenarios.despawn = {
    setup = function(case)
        return {
            entities = evoPopulate(case.params.count, { [PositionE] = true }, function(chunk, b, e)
                local ps = chunk:components(PositionE)
                for i = b, e do
                    ps[i] = { x = i, y = i }
                end
...
evolvedScenarios.removeComp = {
    setup = function(case)
        return {
            entities = evoPopulate(case.params.count, { [PositionE] = true, [VelocityE] = true }, function(chunk, b, e)
                local ps, vs = chunk:components(PositionE, VelocityE)
                for i = b, e do
                    ps[i] = { x = i, y = i }
                    vs[i] = { vx = 1, vy = 1 }
...
evolvedScenarios.valueUpdateHook = {
    setup = function(case)
        return {
            entities = evoPopulate(case.params.count, { [CounterE] = true }, function(chunk, b, e)
...
evolvedScenarios.queryIter = {
    setup = cachedSetup(function(case)
        ...
        evo.multi_spawn_nr(case.params.count, { [PositionE] = true }, function(chunk, b, e)
            local ps = chunk:components(PositionE)
            for i = b, e do
                ps[i] = { x = i, y = i }
            end
...
evolvedScenarios.queryIterScalar = {
    setup = cachedSetup(function(case)
        ...
        evo.multi_spawn_nr(case.params.count, { [ScalarPosXE] = true }, function(chunk, b, e)
...
evolvedScenarios.queryIterFFISoA = {
    setup = cachedSetup(function(case)
        ...
        evo.builder():set(PositionXE):set(PositionYE):multi_spawn(case.params.count, function(chunk, b_place, e_place)
...
evolvedScenarios.queryMulti = {
    setup = cachedSetup(function(case)
        ...
        for arch = 1, 20 do
            local tag = ArchTagsE[arch]
            evo.multi_spawn_nr(case.params.count, { [PositionE] = true, [tag] = true }, function(chunk, b, e)
                local ps = chunk:components(PositionE)
                for i = b, e do
                    ps[i] = { x = i, y = i }
                end
...
evolvedScenarios.queryBuild = {
    ...
    run = function()
        local query = evo.builder():include(PositionE, VelocityE):exclude(DamageE, AggroE):build()
...
evolvedScenarios.sysUpdateScalar = {
    setup = cachedSetup(function(case)
        ...
        evo.multi_spawn_nr(case.params.count, { [ScalarPosXE] = true, [ScalarVelXE] = true }, function(chunk, b, e)
...
evolvedScenarios.dispatch200 = {
    ...
    teardown = function(state)
        for i = 1, #state.systems do
            evo.destroy(state.systems[i])
        end
...
evolvedScenarios.randomGet = {
    setup = cachedSetup(function(case)
        local entities = {}
        evo.multi_spawn_to(entities, 1, case.params.count, { [PositionE] = true }, function(chunk, b, e)
            local ps = chunk:components(PositionE)
            for i = b, e do
                ps[i] = { x = i, y = i }
            end
