...
local PauseState = world:createState("pause")
...
Policy-driven entity mutations stage until the next publication barrier.
The state stack changes immediately.
...
`pause` affects queries with an explicit `Paused` exclusion. Render queries continue to match paused entities. Use
`disable` when entities should stop drawing, and exclude `Disabled` in
game queries.
