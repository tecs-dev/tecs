---
url: /ecs/save-games.md
description: >-
  Snapshot save and load with saveSnapshot, loadSnapshot, transient components,
  snapshot handlers, filtering, and the binary format spec
---
...
* **`tecs.gfx.Sprite`** holds an image's intern index, handed out in the order images were registered.
  A number that depends on load order cannot survive a save, so `serialize` writes the image's *name* and
...
Game code that owns *derived* entities, a projection of smaller durable input, skips them at save time with
