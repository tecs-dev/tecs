---
url: /ecs/components/table-components.md
description: >-
  Lua-table-backed components via newComponent with fields, init, custom __call,
  and new
---
...
*does* fit a C struct, reach for [FFI components](/ecs/components/ffi) instead; they share the same call-site
...
`__call` *and* the `.new` table form. Optional `defaults` fill in static defaults for any field the caller
