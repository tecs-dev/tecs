...
url: /ecs/phases.md
description: >-
  Game-loop phase groups, what the engine runs in each, and the enablePhase,
  disablePhase, registerPhase, and runPhase world methods
---

# Phases

Tecs divides its game loop into *phases*. Add [systems](/ecs/systems) to phases to run game logic at
...
it *after* `world:update` has returned. The `Render`, `PostRender` and `RenderLast` phases still run, and are
