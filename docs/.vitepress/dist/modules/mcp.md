...
url: /modules/mcp.md
description: >-
  The debug server: JSON-RPC over HTTP answered inside the frame, the tools
  every build exposes, the sandbox and the transport
---
...
`tecs.mcp.transport` is HTTP framing over SDL3\_net, polled rather than blocking, because the thing being
...
| Type                       | Description                                                                                                                                                                                            |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| boolean | True when a request was answered, which includes one answered with a 405 or a JSON-RPC error. False when nothing was waiting, when a request is still arriving, or when the server has been destroyed. |
...
snake\_case where the rest of this tree is camelCase; that is
...
| Type                      | Description                                                                                                                         |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| string | nil means no crash has been recorded, which is the only sense in which the game is known to be healthy: nothing here polls for one. |
...
| Type                      | Name                    | Description                                                                                                                                                                         |
| ------------------------- | ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| string | text | One JSON-RPC request object. A batch, that is a JSON array, is not supported; it decodes, has no `method`, and comes back as an invalid-request error rather than being fanned out. |

#### Returns

| Type                      | Description                                                                                                                                                                                        |
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| string | Always a response as JSON text, never nil, and never a raise, since bad input is reported in-band. A request that carried no `id` is still answered, with the `id` field absent from the response. |
...
| Type                       | Name                    | Description                                                                                                                     |
| -------------------------- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| integer | port | Defaults to 7100. A port already in use raises rather than being worked around, since a debugger that moved could not be found. |

#### Returns

| Type                          | Description                                                    |
| ----------------------------- | -------------------------------------------------------------- |
| mcp.Server | Listening, but answering nothing until something calls `poll`. |
...
| Type                        | Name                    | Description                                                                                                                                                                                             |
| --------------------------- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| mcp.Tool | tool | Stored by reference and never copied, so mutating the table afterwards changes what the server dispatches and reports. `name` and `handler` are required and their absence raises; the rest may be nil. |
...
| Type                      | Name                         | Description                                                                                                                                                                                     |
| ------------------------- | ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| string | traceback | Stored as given and handed back verbatim by `crashed`, and sent as the whole body of the error a blocked tool answers with. Passing nil clears the crashed state and lets every tool run again. |
...
| Type                          | Description                                                                                                                                                                   |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| {mcp.Tool} | A fresh list each call, so the caller may keep it. The `Tool` values in it are the registered tables themselves rather than copies, so writing to one writes to the registry. |
