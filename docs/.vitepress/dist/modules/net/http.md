---
url: /modules/net/http.md
description: >-
  An HTTP client where every request is a future, every body is a stream, and
  nothing goes in the frame loop
---
...
There is no `sendSync` and no `blocking = true`, because blocking is something done *to* a future rather than a
