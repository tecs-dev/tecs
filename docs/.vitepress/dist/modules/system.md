---
url: /modules/system.md
description: >-
  The operating system asked directly: what this build can do here, the
  clipboard, running another program, URLs, locales, power and native dialogs
---
...
| Type                       | Description                                                                                                                                              |
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| boolean | Whether the exit code was zero, which is the convention and not a guarantee: a program free to mean something else by it needs `exitCode` read directly. |
...
| Type                      | Name                       | Description                                                                                                                                                                                                                      |
| ------------------------- | -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| string | luaPath | Where the worker finds its modules. Defaults to this state's own, which is right whenever the two run from the same tree. Only read on the call that actually starts the worker, so passing it to a second call changes nothing. |
...
| Type                                                                                    | Name                     | Description                                                                                                           |
| --------------------------------------------------------------------------------------- | ------------------------ | --------------------------------------------------------------------------------------------------------------------- |
| Future\<ProcessResult> | run   | A future `runProcess` handed out. One that has already settled is ignored, so this is safe to call on a race it lost. |
| boolean                                                              | force | Sends the signal a child cannot handle. Defaults to false, which asks and may be refused.                             |
...
| Type                       | Description                                                                                                             |
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| integer | Every run in flight in this process, not one caller's, and counting a child that has been asked to die but has not yet. |
...
| Type                                                                                    | Name                   | Description                                                                         |
| --------------------------------------------------------------------------------------- | ---------------------- | ----------------------------------------------------------------------------------- |
| Future\<ProcessResult> | run | A future `runProcess` handed out. Any other future answers nil rather than raising. |

#### Returns

| Type                      | Description                                                                                                                          |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| Result | The runner's own record, not a copy, so it fills in as the child runs and the same table is what the future carries when it settles. |
...
| Type                       | Description                                                                                                          |
| -------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| boolean | A fact about the process rather than about any world. False costs nothing to act on: `run` starts the worker itself. |
...
| Type                                                                        | Name                       | Description                                                                                                                                                  |
| --------------------------------------------------------------------------- | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| ProcessOptions | options | Read here and sent to the worker, so a later edit to the same table does not reach the child. Starting the runner is part of this call when nothing has yet. |

#### Returns

| Type                                                                                    | Description                                                                                                                                                                                                                                                          |
| --------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Future\<ProcessResult> | A future that settles "ready" with the result when the child ends, however it ended: a non-zero exit is still "ready", and `succeeded` is what distinguishes them. It settles "failed" only when the child could not be started, and "cancelled" when it was killed. |
...
| Type                      | Name                       | Description                                           |
| ------------------------- | -------------------------- | ----------------------------------------------------- |
| number | graceMs | How long a child gets to end gently. Defaults to 250. |
...
| Type                       | Description             |
| -------------------------- | ----------------------- |
| integer | How many runs finished. |
