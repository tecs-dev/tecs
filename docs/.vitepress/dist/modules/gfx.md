...
url: /modules/gfx.md
description: >-
  Drawing: the camera, the render components, the renderer, text, and the
  vocabularies a scene is described in
---
...
record it hands back describes a material *file*, while `Material` is the component that selects one by
...
| ------------------------- | ------------------------- | -------------------------- |
| Camera | self   |                            |
| number | width  | Viewport width in pixels.  |
| number | height | Viewport height in pixels. |
...
| Type                             | Name                       | Description                                                                      |
| -------------------------------- | -------------------------- | -------------------------------------------------------------------------------- |
| CameraOptions | options | Omit for the default view. No field is validated; the values are taken as given. |

#### Returns

| Type                      | Description                                                                           |
| ------------------------- | ------------------------------------------------------------------------------------- |
| Camera | A camera whose fields are plain and meant to be assigned to directly, frame by frame. |
...
| ------------------------- | ------------------------- | ------------------------------------------------------------------ |
| Camera | self   |                                                                    |
| number | worldX | World x.                                                           |
| number | worldY | World y, running down.                                             |
| number | width  | Viewport width in pixels, the same one the matrix was built with.  |
| number | height | Viewport height in pixels, the same one the matrix was built with. |

#### Returns

| Type                      | Description                                                                                                         |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| number | The screen x in pixels from the left, then the screen y in pixels from the top. Neither is clamped to the viewport. |
| number |                                                                                                                     |
...
| ------------------------- | -------------------------- | ------------------------------------------------------------------ |
| Camera | self    |                                                                    |
| number | screenX | Pixels from the left of the viewport.                              |
| number | screenY | Pixels from the top of the viewport, running down.                 |
| number | width   | Viewport width in pixels, the same one the matrix was built with.  |
| number | height  | Viewport height in pixels, the same one the matrix was built with. |
...
| Type                      | Name                    | Description                                                           |
| ------------------------- | ----------------------- | --------------------------------------------------------------------- |
| string | name | Must be non-empty; an empty or nil name errors rather than interning. |
...
| Type                       | Name                  | Description                                  |
| -------------------------- | --------------------- | -------------------------------------------- |
| integer | id | An index previously handed out by `imageId`. |

#### Returns

| Type                      | Description                                               |
| ------------------------- | --------------------------------------------------------- |
| string | The normalised name, or nil when the index names nothing. |
...
| ----------------------------------------------------------------------------- | ------------------------ | -------------------------------------------------------------------------------- |
| Renderer                                                   | self  |                                                                                  |
| ComputeStage | stage | Held until `destroy`, which calls its own. There is no way to take one back out. |
...
| ------------------------------------------------------------------------------------- | --------------------------- | ------------------------------------------------------------------------ |
| Renderer                                                           | self     |                                                                          |
| InstanceProducer | producer | Held for the life of the renderer; there is no way to take one back out. |
...
| Type                       | Description                                                                                                                                                                                             |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Texture | The scene target rather than the swapchain, so it can be read at any point between frames. The renderer goes on drawing into it, so the next frame overwrites what a caller has not already copied out. |
...
| --------------------------- | ------------------------ | ------------------------------------------------------------------------------ |
| Renderer | self  |                                                                                |
| integer  | index | The same range `setClipRegion` takes. Clearing one nobody set is not an error. |
...
| Type                      | Description                                                                                                         |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| number | Zero, or world units, which is also the factor `layers.maxY` and `layers.maxZ` divide by to resolve the sort again. |
...
| Type                           | Description                                                                                                                  |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------- |
| loader.CPtr | The `SDL_GPUDevice` handed to `create`. Not owned here: `destroy` releases what was built on it and never the device itself. |
...
| ---------------------------- | ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Renderer  | self  |                                                                                                                                                             |
| ecs.World | world | The queries and both systems are added here and nowhere else, so a renderer that was created and never installed draws an empty packet rather than failing. |
...
| ---------------------------------- | ---------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| loader.CPtr     | device          |                                                                                                                |
| integer         | swapchainFormat | The `SDL_GPUTextureFormat` present copies into.                                                                |
| RendererOptions | options         | Every field has a default, and nil is accepted at runtime even though the signature does not mark it optional. |
...
| --------------------------- | ----------------------- | -------------------------------------------------------------- |
| Renderer | self |                                                                |
| string   | path | The path the image was registered under, spelled the same way. |

#### Returns

| Type                                   | Description                                                                                                                                                                            |
| -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TextureArray.Region | The renderer's own record of where the image landed, not a copy. Nil says nothing is registered under that path, which is the one answer `registerImage` and `replaceImage` differ on. |
...
| -------------------------------- | ------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| Renderer      | self   |                                                                                                                        |
| assets.Handle | handle | Must have loaded: any other status raises, and the pixels are released whether the path was new or already registered. |

#### Returns

| Type                                   | Description                                                                                                              |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| components.Sprite   | A Sprite selecting the whole image, ready to spawn.                                                                      |
| TextureArray.Region | Where it landed, which reports the size that was uploaded rather than the cell's. The renderer's own record, not a copy. |
...
| --------------------------- | ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Renderer | self  |                                                                                                                                                                                                                                                           |
| Frame    | frame | Recorded onto and not submitted, so a caller with passes of its own records them after this and submits once. Its width and height are what the camera's matrix and the view bounds are resolved against, which is why they are not stored on the camera. |
...
| -------------------------------- | ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Renderer      | self   |                                                                                                                                                                                               |
| assets.Handle | handle | Must have loaded, must name a path already registered, and must be the same size it was registered at. Each of the three raises, and the pixels are only released once all three have passed. |

#### Returns

| Type                                   | Description                                                                                |
| -------------------------------------- | ------------------------------------------------------------------------------------------ |
| components.Sprite   | The same Sprite `registerImage` answered with, since neither the layer nor the rect moved. |
| TextureArray.Region | The region, unchanged, and still the renderer's own record.                                |
...
| ------------------------------------------------------------------------- | ------------------------- | ----------------------------------------------------------------------------------- |
| Renderer                                               | self   |                                                                                     |
| integer                                                | index  | 1 through 255. Zero, a fraction, or anything outside that raises.                   |
| ClipRegion | region | Nil raises rather than clearing; `clearClipRegion` is what stops a region clipping. |
...
| --------------------------- | ----------------------- | --------------------------------------------------------------------------------------------------- |
| Renderer | self |                                                                                                     |
| string   | name | The path the image was registered under.                                                            |
| number   | u0   | Zero at the image's left edge and one at its right, not the cell's. Defaults to zero, as `v0` does. |
| number   | v0   |                                                                                                     |
| number   | u1   | Defaults to one, as `v1` does, so omitting all four is the whole image.                             |
| number   | v1   |                                                                                                     |
...
| Type                                 | Description                                                                                         |
| ------------------------------------ | --------------------------------------------------------------------------------------------------- |
| components.Sprite | A fresh Sprite, with the fractions already mapped into the rect the image occupies within its cell. |
...
| Type                                                 | Description                                                                    |
| ---------------------------------------------------- | ------------------------------------------------------------------------------ |
| Font | The same font every call, since it goes through `loadFont` under a fixed path. |
...
| Type                      | Name                    | Description                                                                   |
| ------------------------- | ----------------------- | ----------------------------------------------------------------------------- |
| string | name | The path exactly as it was passed to `loadFont`, not the file it resolves to. |

#### Returns

| Type                                                 | Description                                                                                           |
| ---------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| Font | Nil when nothing has loaded that path, which is the non-raising way to ask; `loadFont` would read it. |
...
| World   | world  |                                                                                                                                                                                          |
| integer | entity |                                                                                                                                                                                          |
| integer | index  | One-based over the glyphs the layout produced, which is not the string's byte index: a newline, a space, and a codepoint the font has no metrics for each move the pen and take no slot. |

#### Returns

| Type                      | Description                                                                                                                                                                                                                    |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| number | World x of the glyph's centre, then y, then its width and height in world units. All four are nil together when the plugin is not installed on the world, the entity carries no `Text`, or the index is past what it laid out. |
| number |                                                                                                                                                                                                                                |
| number |                                                                                                                                                                                                                                |
| number |                                                                                                                                                                                                                                |
...
| Type                                                               | Name                       | Description                                                                                                                                              |
| ------------------------------------------------------------------ | -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FontOptions | options | `metrics` is required. `atlas` defaults to the first page the metrics name, resolved beside them, and is only consulted the first time a path is loaded. |

#### Returns

| Type                                                 | Description                                                                                                                                                     |
| ---------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Font | The one font for that path, shared rather than copied. The atlas is named and not read here: a renderer decodes and uploads it the first frame a text needs it. |
...
| Type                                                 | Name                    | Description                                                                                               |
| ---------------------------------------------------- | ----------------------- | --------------------------------------------------------------------------------------------------------- |
| Text | item | Read and not written, and it need not be on an entity: a `Text` assembled to ask this question is enough. |

#### Returns

| Type                      | Description                                                                                                                                                  |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| number | Width in world units at `item.size`, before any Transform scale the entity drawing it applies. Zero for a nil item, one with no font, or one with no string. |
| number | Height on the same terms, counting whole lines: a one-line text is one line height rather than the ink's extent.                                             |
...
| Type                      | Name                    | Description                                                               |
| ------------------------- | ----------------------- | ------------------------------------------------------------------------- |
| string | path | The metrics path the font was loaded under, or the file it was read from. |

#### Returns

| Type                                                 | Description                                 |
| ---------------------------------------------------- | ------------------------------------------- |
| Font | The font, which is the table it always was. |
...
| Type                       | Description                                                                                                                    |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| integer | A count that only ever increases, and zero for a world the plugin was never installed on. It counts rows laid out, not frames. |
...
| Type                                                               | Name                       | Description                                   |
| ------------------------------------------------------------------ | -------------------------- | --------------------------------------------- |
| TextOptions | options | `renderer` is required and is the only field. |

#### Returns

| Type                               | Description                                                                                                                                                                                 |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| function(World) | The plugin, to be handed to a world. Each world it is installed on gets its own producer run of the one renderer's instance buffer, and the fonts themselves are shared across all of them. |
