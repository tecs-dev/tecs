...
`nil` encodes as `null`, and so does [`data.null`](#null). A table *field* set to `nil` is not a field at all,
...
### array\_mt and empty\_array\_mt {#array-mt-and-empty-array-mt}
...
The four that change what a payload *means* rather than how it looks are worth knowing by name:
...
Every function and type this module carries, rendered from `src/tecs/data.tl`.
...
| -------------------------- | ------------------------ | ---------------------------------------------------------------------------------------------------------------- |
| string  | bytes |                                                                                                                  |
| integer | level | Minus one or omitted uses zlib's default. Zero stores without compression; one through nine trade time for size. |
...
| Type                      | Name                    | Description                                              |
| ------------------------- | ----------------------- | -------------------------------------------------------- |
| string | text | Any string. NUL and bytes above 127 hash like any other. |

#### Returns

| Type                      | Description                                                                                                |
| ------------------------- | ---------------------------------------------------------------------------------------------------------- |
| string | Sixteen hex digits, high half first, so hashes sort as their values do and two of them compare as strings. |
...
| Type                       | Name                        | Description                                                                                                                    |
| -------------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| string  | bytes    | The whole stream, header and Adler-32 trailer included. An empty string raises, since it is not one.                           |
| integer | sizeHint | The decompressed size, when the caller's format records it. Only an allocation hint: a wrong one costs a copy, never an error. |

#### Returns

| Type                      | Description                                                                                                                                      |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| string | The decompressed bytes, which may be empty. Never nil: a bad header, a corrupt block, a truncated stream and a trailer that disagrees all raise. |
...
| Type                       | Name                        | Description                                                                                                                    |
| -------------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| string  | bytes    | The whole stream, compressed. An empty string raises, since it is not one.                                                     |
| integer | sizeHint | The decompressed size, when the caller's format records it. Only an allocation hint: a wrong one costs a copy, never an error. |
...
| Type                      | Description                                                                                  |
| ------------------------- | -------------------------------------------------------------------------------------------- |
| string | The decompressed bytes, which may be empty. Never nil: a stream that does not decode raises. |
