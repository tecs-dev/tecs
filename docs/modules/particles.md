...
local particles <const> = tecs.particles
```
...
[sheet](/modules/sheet). A `layer` outside one to [`layers.MAX`](/modules/layers), and an `alignment`
...
have, for the same trade the frame table already made: the public surface stays a curve with keys
...
to each other, which is the only reason they are on the surface.

## Design record

- [Particles are emitters, not entities](https://github.com/tecs-dev/tecs/blob/main/README.md#particles-are-emitters-not-entities)
