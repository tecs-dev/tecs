...
the aggregator that pulls every engine module in, so a module the surface exports cannot also depend on the
surface without making a cycle, which Teal rejects even through a type-only require. Engine modules therefore
...
The ECS half loads eagerly, because it reaches nothing below Lua. The engine half is named in the surface for its
