...
The whole surface is `require("tecs")` and every module is a field on it, so this module is `tecs.mcp`. `tecs`
is also set as a global, which makes the require line optional, and engine modules such as this one are
resolved lazily on first field access. The submodules are reached by path:
`require("tecs.mcp.tools")`, `require("tecs.mcp.world")`, `require("tecs.mcp.transport")` and
`require("tecs.mcp.sandbox")`.
...
- [What the shape costs](https://github.com/tecs-dev/tecs/blob/main/README.md#what-the-shape-costs)
