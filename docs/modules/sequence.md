...
The whole surface is `require("tecs")` and every module is a field on it, so this module is `tecs.sequence`.
...
The same authoring surface as a list of steps, for callers that cannot invoke Lua: a tool over the debug
...
- [Status](https://github.com/tecs-dev/tecs/blob/main/README.md#status)
