---
description: "Type-safe Love event wrappers, routed interaction ownership, input capture events, and global lifecycle delivery"
outline: deep
---
...
Triggered when Love produces text according to the active keyboard layout. Use this for text fields rather than
...
| `direction` | `love.mouse.WheelDirection` | Love 12 wheel direction, such as `"standard"` or `"flick"`. |
...
snapshot-managed input topology, not by a Love callback. Each event is sent directly to the target losing or gaining
