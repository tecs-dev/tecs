...
//
// One invocation advances each emitter's own cursor without a shared atomic. Order is not
...
layout(set = 0, binding = 3) buffer Runtime { uint value[]; } runtime;
...
void main() {
    ...
    bool fresh = runtime.value[at + RUNTIME_GENERATION] != generation;
    if (fresh) {
    ...
    float spawn = 0.0;
    // Snapshots restore an empty field at its saved clock. Bursts before that
    // clock have already happened and must not be replayed on GPU admission.
    float previousAge = (clock - steps - startStep) * dt - delay;
    if (fresh && previousAge > 0.0) {
        float previousCycleTime = previousAge;
        if (duration > 0.0 && looping != 0.0) {
            cycleAt = uint(floor(previousAge / duration));
            previousCycleTime -= float(cycleAt) * duration;
        }
        for (int index = 0; index < EFFECT_MAX_BURSTS; index++) {
            if (index >= bursts) { break; }
            if (effects.value[effect + EFFECT_BURSTS + index * 2] <= previousCycleTime) {
                fired |= (1u << uint(index));
            }
        }
    }
    ...
    runtime.value[at + RUNTIME_SPAWN_CURSOR] = runtime.value[at + RUNTIME_CURSOR];
    runtime.value[at + RUNTIME_CURSOR] += uint(spawn);
