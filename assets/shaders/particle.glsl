...
// times.
//
// The host half is src/tecs/gfx/particles.nupp, which writes all three buffers
// and holds the same numbers. The pair only works while they agree, and the
// native GPU and Nupp emitter tests hold the two sides to the same contract.
//
layout(set = 0, binding = 0) readonly buffer Effects {
...
layout(set = 0, binding = 1) readonly buffer Emitters {
    float value[];
} emitters;

// Floats per effect record. `EFFECT_FLOATS` in src/tecs/gfx/particles.nupp is
// the same number.
const int PARTICLE_EFFECT_FLOATS = 69;
