...
// Particle records and lookup functions are prepended from particle.glsl.


layout(set = 0, binding = 2) readonly buffer Owners { uint value[]; } owners;

layout(set = 0, binding = 4) buffer States { float value[]; } states;

// The same eighty-byte instance consumed by the ordinary WGSL sprite path.
struct Instance { vec4 position; vec4 scale; vec4 uvRect; vec4 color; uint material; uint flags; uint clipMin; uint clipMax; };
layout(set = 0, binding = 5) buffer Instances { Instance item[]; } instances;

layout(set = 0, binding = 6) uniform Particles {
...
void particleHide(uint slot) {
    ...
    instances.item[index].position = vec4(PARTICLE_HIDDEN, PARTICLE_HIDDEN, 0.0, PARTICLE_HIDDEN_DEPTH);
    instances.item[index].scale = vec4(0.0);
    instances.item[index].color = vec4(0.0);
    instances.item[index].uvRect = vec4(0.0);
    instances.item[index].material=0u; instances.item[index].flags=0u;
    instances.item[index].clipMin=0u; instances.item[index].clipMax=0u;
}
...
void main() {
    ...
    if (birth < emitters.value[emitter + EMITTER_CLEAR_STEP]) {
        states.value[state + STATE_LIFETIME] = 0.0;
        particleHide(slot);
        return;
    }
    ...
    vec2 drawn = position + carried * follows
        + velocity * (params.timing.w * timestep * emitters.value[emitter + 23]);
    ...
    if (playback > 0.0) {
        ...
        region = vec4(-playback, effects.value[effect + EFFECT_PLAYBACK_TICKS] * age / lifetime, 0.0, 0.0);
    }

    uint index = uint(params.pool.x) + slot;
    instances.item[index].position = vec4(drawn, rotation, effects.value[effect + EFFECT_DEPTH]);
    float material = effects.value[effect + EFFECT_MATERIAL];
    instances.item[index].scale = vec4(scaleX, scaleY, fract(material), 0.0);
    instances.item[index].uvRect = region;
    instances.item[index].color = tint;
    instances.item[index].material = uint(floor(material));
    float blend = effects.value[effect + EFFECT_BLENDED];
    instances.item[index].flags = blend > 0.0 ? 1u : 0u;
    if (blend == 2.0) { instances.item[index].flags |= 64u; }
    instances.item[index].flags |= 8u;
    if (effects.value[effect + EFFECT_UNBOUNDED] != 0.0) { instances.item[index].flags |= 128u; }
    instances.item[index].clipMin = uint(effects.value[effect + 65]) | (uint(effects.value[effect + 66]) << 16u);
    instances.item[index].clipMax = uint(effects.value[effect + 67]) | (uint(effects.value[effect + 68]) << 16u);
}
