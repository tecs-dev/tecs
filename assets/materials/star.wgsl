...
fn starDistance(point: vec2<f32>, outer: f32, inner: f32) -> f32 {
    let sector = 3.14159265 / 5.0;
    // Measured from -Y so a tip points up. WGSL has no `mod`, so the wrap is
    // written out; it is the same fold the GLSL original used.
    let raw = atan2(point.x, -point.y) + sector;
    let angle = raw - 2.0 * sector * floor(raw / (2.0 * sector)) - sector;
    let folded = length(point) * vec2<f32>(cos(angle), abs(sin(angle)));

    let tip = vec2<f32>(outer, 0.0);
    let edge = inner * vec2<f32>(cos(sector), sin(sector)) - tip;
    let offset = folded - tip;
    let along = clamp(dot(offset, edge) / dot(edge, edge), 0.0, 1.0);
    // The cross product is positive on the side the center is on, so it is the
    // sign the distance takes.
    let side = sign(edge.x * offset.y - edge.y * offset.x);
