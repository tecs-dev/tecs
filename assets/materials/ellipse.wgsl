...
fn ellipseDistance(point: vec2<f32>, radii: vec2<f32>) -> f32 {
    let scaled = length(point / radii);
    let gradient = length(point / (radii * radii));
...
fn material(frag: MaterialInput) -> MaterialOutput {
    ...
    let radii = vec2<f32>(0.5, max(0.5 * frag.param, 1e-4));
