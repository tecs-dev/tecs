...
function Server:cliTools()
    ...
    return {
        ...
        {
            name = "build",
            description = "Compile the game into build/. A running game hot-reloads the result "
                .. "automatically; no restart is needed for code or asset changes.",
            inputSchema = OBJECT_SCHEMA,
            handler = function()
                ctx.build()
        ...
        {
            name = "integ",
            description = "Run spec/ with the bundled busted runner; *_lovespec.tl specs boot "
                .. "the built game and drive it over MCP.",
            inputSchema = OBJECT_SCHEMA,
            handler = function()
                local ok, output = ctx.reexec("integ")
                return {ok = ok, output = output}
            end,
        },
        {
            name = "dist",
            description = "Package the built game (.love, macOS app bundle, Windows executable) "
                .. "into dist/.",
            inputSchema = {
                type = "object",
                properties = {
                    target = {
                        type = "string",
                        enum = {"love", "macos", "windows"},
                        description = "Optional single target; omit for all.",
                    },
                },
            },
            handler = function(args)
                local ok, output = ctx.reexec("dist", args and args.target)
                return {ok = ok, output = output}
            end,
        },
        {
