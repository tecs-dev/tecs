//! Packaging for the Nupp and Rust path: presets, installs, and the gate on
...
//! built it.
//!
//! Two preset kinds hold that line the way `crate::presets` holds it for the
//! Teal path. A development preset links against the staged SDK where it sits
//! and ships no shader pack, which is convenient and not shippable. A release
...
/// Where an installed Nupp release is written.
///
/// It is deliberately not `out/package`, which the Teal path owns. The two
/// implementations are still parallel and a run of one must not overwrite the
/// other's install.
pub const OUTPUT: &str = "out/nupp-package";
    ...
    let application_search_paths: Vec<_> = if platform == Platform::Macos {
        inspections
            ...
            .flat_map(|(binary, (rpaths, _))| {
                rpaths
                    .iter()
                    .filter_map(|rpath| crate::package::resolved_search_path(prefix, binary, rpath))
            })
            .collect()
    } else {
        Vec::new()
