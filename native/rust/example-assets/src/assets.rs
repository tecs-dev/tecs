...
pub fn require_for_example(root: &Path, name: &str) -> Result<()> {
    if name == "sponza3d" && !current_large_scene_cache(root, "sponza", "Sponza.tecs.gltf") {
        bail!(
            "the sponza3d example needs its ignored scene cache; run `nupp task fetch sponza` first"
        );
    }
    if name == "bistro3d" && !current_large_scene_cache(root, "bistro", "Bistro.tecs.gltf") {
        bail!(
            "the bistro3d example needs its ignored scene cache; run `nupp task fetch bistro` first"
        );
    }
    Ok(())
}

#[cfg(any(test, feature = "bistro-import"))]
...
fn fetch_bistro(_root: &Path) -> Result<()> {
    bail!("Bistro needs the optional importer; run `nupp task fetch bistro`");
}
...
fn encode_bc3_ktx2(
    ...
    let mut kvd = Vec::new();
    push_kvd(&mut kvd, "KTXwriter", b"Tecs example assets\0");
