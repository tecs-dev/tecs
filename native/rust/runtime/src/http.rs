...
fn optional_uri_part(value: Option<&str>, length: *mut usize) -> *const u8 {
    match value {
        Some(value) => event_data(value.as_bytes(), length),
        None => {
            if !length.is_null() {
...
/// Parses one absolute URI through `url::Url`.
///
/// # Safety
///
/// `text` must be null with zero length or readable for `length` bytes.
#[no_mangle]
pub unsafe extern "C" fn tecsHttpUriParse(text: *const u8, length: usize) -> *mut TecsHttpUri {
    let bytes = if text.is_null() {
        if length == 0 {
            &[][..]
        } else {
            set_error("a non-empty URI has a null pointer");
            return ptr::null_mut();
        }
    } else {
        // SAFETY: The caller promises readable input for this synchronous call.
        unsafe { slice::from_raw_parts(text, length) }
    };
    let text = match str::from_utf8(bytes) {
        Ok(text) => text,
        Err(_) => {
            set_error("URI is not UTF-8");
            return ptr::null_mut();
        }
    };
    match Url::parse(text) {
        Ok(url) => Box::into_raw(Box::new(TecsHttpUri { url })),
        Err(error) => {
            set_error(error);
            ptr::null_mut()
        }
...
#[no_mangle]
pub unsafe extern "C" fn tecsHttpUriText(uri: *const TecsHttpUri, length: *mut usize) -> *const u8 {
    if uri.is_null() {
        return optional_uri_part(None, length);
    }
    // SAFETY: Null was rejected and the caller promises a live URI.
    optional_uri_part(Some(unsafe { &*uri }.url.as_str()), length)
}

#[no_mangle]
pub unsafe extern "C" fn tecsHttpUriScheme(
    uri: *const TecsHttpUri,
    length: *mut usize,
) -> *const u8 {
    if uri.is_null() {
        return optional_uri_part(None, length);
    ...
    // SAFETY: Null was rejected and the caller promises a live URI.
    optional_uri_part(Some(unsafe { &*uri }.url.scheme()), length)
}

#[no_mangle]
pub unsafe extern "C" fn tecsHttpUriAuthority(
    uri: *const TecsHttpUri,
    length: *mut usize,
) -> *const u8 {
    if uri.is_null() {
    ...
    // SAFETY: Null was rejected and the caller promises a live URI.
    optional_uri_part(authority(&unsafe { &*uri }.url), length)
}

#[no_mangle]
pub unsafe extern "C" fn tecsHttpUriUsername(
    uri: *const TecsHttpUri,
    length: *mut usize,
) -> *const u8 {
    if uri.is_null() {
    ...
    // SAFETY: Null was rejected and the caller promises a live URI.
    optional_uri_part(Some(unsafe { &*uri }.url.username()), length)
}

#[no_mangle]
pub unsafe extern "C" fn tecsHttpUriPassword(
    uri: *const TecsHttpUri,
    length: *mut usize,
) -> *const u8 {
    if uri.is_null() {
    ...
    // SAFETY: Null was rejected and the caller promises a live URI.
    optional_uri_part(unsafe { &*uri }.url.password(), length)
}

#[no_mangle]
pub unsafe extern "C" fn tecsHttpUriHost(uri: *const TecsHttpUri, length: *mut usize) -> *const u8 {
    if uri.is_null() {
        return optional_uri_part(None, length);
    }
    // SAFETY: Null was rejected and the caller promises a live URI.
    optional_uri_part(unsafe { &*uri }.url.host_str(), length)
}

#[no_mangle]
pub unsafe extern "C" fn tecsHttpUriPort(uri: *const TecsHttpUri, port: *mut u16) -> i32 {
    if uri.is_null() {
        return 0;
    }
    // SAFETY: Null was rejected and the caller promises a live URI.
...
#[no_mangle]
pub unsafe extern "C" fn tecsHttpUriPath(uri: *const TecsHttpUri, length: *mut usize) -> *const u8 {
    if uri.is_null() {
        return optional_uri_part(None, length);
    }
    // SAFETY: Null was rejected and the caller promises a live URI.
    optional_uri_part(Some(unsafe { &*uri }.url.path()), length)
}

#[no_mangle]
pub unsafe extern "C" fn tecsHttpUriQuery(
    uri: *const TecsHttpUri,
    length: *mut usize,
) -> *const u8 {
    if uri.is_null() {
    ...
    // SAFETY: Null was rejected and the caller promises a live URI.
    optional_uri_part(unsafe { &*uri }.url.query(), length)
}

#[no_mangle]
pub unsafe extern "C" fn tecsHttpUriFragment(
    uri: *const TecsHttpUri,
    length: *mut usize,
) -> *const u8 {
    if uri.is_null() {
    ...
    // SAFETY: Null was rejected and the caller promises a live URI.
    optional_uri_part(unsafe { &*uri }.url.fragment(), length)
}

#[no_mangle]
pub unsafe extern "C" fn tecsHttpUriDestroy(uri: *mut TecsHttpUri) {
    if !uri.is_null() {
