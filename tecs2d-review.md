...
- [x] **Drop-shadow fan-out writes past buffer capacity.** Fixed: `bucket_sync` sizes `dropShadowOutputBuffer` at `capacity * 8` (matching the non-provider heuristic) and tracks `dropShadowCapacity`; `shadow_dispatch` sends the bucket's real capacity as `MaxDropShadows` next to the output binding (constant 65536 sends removed from `sprite/init.tl` and `render_plugin.tl`). Needs a visual check with drop shadows + multiple lights.
