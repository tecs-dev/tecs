...
- [x] **Drop-shadow fan-out writes past buffer capacity.** Fixed: `bucket_sync` sizes `dropShadowOutputBuffer` at `capacity * 8` and tracks `dropShadowCapacity`; `sprite/shadow_dispatch` sends the bucket's real capacity as `MaxDropShadows` next to the output binding (constant 65536 sends removed). Follow-up: the dead non-provider bucket-ownership path in `sprite/shadow_dispatch` (own buffers, `getBucket`/`liveBuckets`, always-empty indirect-reset loop) was removed entirely; `bucketProvider` is now required. Needs a visual check with drop shadows + multiple lights.
- [x] **TextEffects std430 stride mismatch (68 vs 80 bytes).** Fixed: all three layers (FFI cdef, LÖVE BufferFormat, GLSL struct) are now all-scalar at the natural 68-byte stride; a new std430 spec asserts every layout's BufferFormat lane sum matches its FFI struct size.
- [x] **Blend-batch cull binds a 1-element dummy as `MaterialParamsOutput`.** Fixed by removing the whole blend-batch machinery: it was write-orphaned (created empty in `shadow_plugin`, populated nowhere; live blend routing is packed BlendId + `blendGroups` forward pass). Removed `blendBatches` from `shape_utils`/`shadow_plugin`, the `renderGBufferBlend` hook, and the dead `DualBlendBatch`/`BlendKey` types; ellipse/arc shader comments updated to state that non-alpha blend routing doesn't exist for them yet (still an open P1).
- [x] **Buffer grow without pinning (known UAF class).** Fixed: new `gpu/retire.tl` holds replaced GPU objects two frames before `release()` (ticked once per frame from `Renderer:upload`), applied to `gpu/buffer.tl`, `shadow_buffer`, `shadow_plugin`, `sprite/bucket_sync`, `mesh/definition`, `tilechunk`, `timing_buffer`, and `physicsDebug` (mesh + ByteData). Unit spec added. Note: `buffer.new` still has no initial-capacity floor (now only a perf concern, not a safety one); `render_column.tl` untouched (dead, see P3).
- [x] **CPU Draw-phase content renders twice when screen layers exist.** Fixed: `_renderPrimary` narrows `currentLayerMin/Max` to the world layers in phase 1 and the screen layers in phase 2 (restored after); the G-buffer draw callback now receives rotation from the render pass instead of reading the world camera, so screen-phase CPU draws are rotation-free; layer-effect groups are clamped to `worldMaxLayer` (groups entirely above it are skipped). Needs a visual check with screen layers + Draw-phase content + a rotated camera.
- [x] **Tiled crashes.** Fixed: `toJSON` copies the FFI tile buffer into a plain GID array; infinite maps error at parse time with a clear message (dense-chunk stitching would be feature work if ever wanted); csv-declared array data routes to the raw-array path; string CSV writes are bounded and entry counts validated against width * height (a short raw array previously allowed FFI reads past the allocation too). Covered by the new `spec/tecs2d/tiled/parser_spec.tl`.
