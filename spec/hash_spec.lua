...
describe("hash.fnv1a64", function()
    it("matches the published reference vectors", function()
        assert.are.equal("cbf29ce484222325", hash.fnv1a64(""))
        assert.are.equal("af63dc4c8601ec8c", hash.fnv1a64("a"))
        assert.are.equal("af63df4c8601f1a5", hash.fnv1a64("b"))
        assert.are.equal("af63de4c8601eff2", hash.fnv1a64("c"))
        assert.are.equal("af63d94c8601e773", hash.fnv1a64("d"))
        assert.are.equal("85944171f73967e8", hash.fnv1a64("foobar"))
    end)
    ...
    it("matches them across the boundary of the block it reads", function()
        ...
        assert.are.equal("a430d84680aabd0b", hash.fnv1a64("hello"))
        assert.are.equal("7396e6eb4e2040df", hash.fnv1a64("foobarfo"))
        assert.are.equal("89ab11d5c0cdeb10", hash.fnv1a64("foobarfoo"))
    end)

    it("is sixteen hex digits, high half first", function()
        local digest = hash.fnv1a64("#version 450\nvoid main() {}\n")
        ...
        assert.are.equal("cbf29ce4" .. "84222325", hash.fnv1a64(""))
        assert.is_true(hash.fnv1a64("foobar") < hash.fnv1a64("a"))
    end)
    ...
    it("hashes bytes rather than characters", function()
        ...
        assert.are_not.equal(hash.fnv1a64("a\0b"), hash.fnv1a64("a"))
        assert.are_not.equal(hash.fnv1a64("\255"), hash.fnv1a64("\127"))
        assert.are.equal("af64724c8602eb6e", hash.fnv1a64("\255"))
        assert.are.equal("e5d29919042666b2", hash.fnv1a64("a\0b"))
    end)

    it("separates inputs that differ in one byte", function()
        local one = hash.fnv1a64("#version 450\nvoid main() {}\n")
        assert.are.equal(one, hash.fnv1a64("#version 450\nvoid main() {}\n"))
        assert.are_not.equal(one, hash.fnv1a64("#version 450\nvoid main(){}\n"))
    end)
end)

describe("hash.adler32", function()
    it("matches the values RFC 1950's definition produces", function()
        assert.are.equal(0x00000001, hash.adler32(""))
        assert.are.equal(0x00620062, hash.adler32("a"))
        assert.are.equal(0x024d0127, hash.adler32("abc"))
        assert.are.equal(0x11e60398, hash.adler32("Wikipedia"))
    end)
    ...
    it("agrees over an input that spans several deferred runs", function()
        ...
        assert.are.equal(0x8454d48d, hash.adler32(long))
    end)
...
describe("hash.crc32", function()
    it("matches the published check value", function()
        assert.are.equal(0, hash.crc32(""))
        assert.are.equal(0xcbf43926, hash.crc32("123456789"))
    end)

    it("hashes every byte and returns a Lua number", function()
        assert.are.equal(0x352441c2, hash.crc32("abc"))
        assert.are.not_equal(hash.crc32("a\0b"), hash.crc32("a"))
        local checksum = hash.crc32(string.rep("\255", 8192))
        assert.are.equal("number", type(checksum))
        assert.is_true(checksum >= 0 and checksum < 4294967296)
    end)
end)
