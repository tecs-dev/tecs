...
describe("platform.filesystem", function()
    ...
    describe("creating and writing", function()
        ...
        it("writes bytes rather than text", function()
            ...
            assert.are.equal(blob, assets.read(at("blob.bin")))
        end)
        ...
        it("pairs with assets.read, which is the reader", function()
            ...
            assert.are.equal('{"score":41}', assets.read(save))
            assert.are.equal("document", assets.loaded()[save], "the read is recorded for the watcher")
        end)
        ...
        it("replaces what was there rather than appending", function()
            ...
            assert.are.equal("short", assets.read(at("over.txt")))
        end)
    ...
    describe("renaming and copying", function()
        it("moves a file", function()
            ...
            assert.are.equal("carried", assets.read(at("to.txt")))
        end)
        ...
        it("overwrites an existing destination without asking", function()
            ...
            assert.are.equal("new", assets.read(at("old.txt")))
        end)
        ...
        it("moves a directory, contents and all", function()
            ...
            assert.are.equal("leaf", assets.read(at("after/inner/leaf.txt")))
        end)
        ...
        it("copies a file, leaving the original", function()
            ...
            assert.are.equal("duplicated", assets.read(at("source.txt")))
            assert.are.equal("duplicated", assets.read(at("dest.txt")))
        end)
