...
local function recording(body)
    ...
    local ok, reason = pcall(function()
        for _, name in ipairs(WRAPPED) do modules[name] = require(name) end
        body(modules)
    end)

    sdl.C = C
    for _, name in ipairs(WRAPPED) do package.loaded[name] = saved[name] end
...
describe("gpu debug names", function()
    ...
    it("hands SDL the name a texture was created with", function()
        local named = recording(function(modules)
            ...
            local anonymous = Texture.create(device.handle,
                { width = SIZE, height = SIZE, format = FORMAT })
            assert.is_nil(anonymous.name)
            anonymous:destroy()
        end)

        assert.are.same({ "readback" }, named.textures,
            "the named texture, and no call for the one without a name")
    end)

    it("hands SDL the name a buffer was created with", function()
        local named = recording(function(modules)
            local Buffer = modules["tecs.gpu.Buffer"]
            local labelled = Buffer.create(device.handle,
                { usage = { "storage" }, size = 256, name = "instances" })
            assert.are.equal("instances", labelled.name)
            labelled:destroy()

            local anonymous = Buffer.create(device.handle,
                { usage = { "storage" }, size = 256 })
            assert.is_nil(anonymous.name)
            anonymous:destroy()
        end)
    ...
    it("names every target the pass graph owns, and the depth attachment",
        function()
            ...
            local named = recording(function(modules)
                ...
                graph:execute({
                    width = SIZE, height = SIZE, commandBuffer = commandBuffer,
                })
                assert(C.SDL_SubmitGPUCommandBuffer(commandBuffer))
                graph:destroy()
            end)

            assert.are.same({ "albedo", "normal", "depth" }, named.textures,
