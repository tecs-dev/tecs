...
describe("io.files", function()
    ...
    describe("creating and writing", function()
        ...
        it("appends binary bytes and creates an absent file", function()
            ...
            local file = assert(files.open(path, "a"))
            assert.is_true(file:write("first\0"))
            assert.is_true(file:write("\255last"))
            assert.is_true(file:close())
            assert.are.equal("first\0\255last", files.read(path))

            file = assert(files.open(at("empty.bin"), "a"))
            assert.is_true(file:close())
            assert.are.equal(0, files.info(at("empty.bin")).size)
        end)
        ...
        it("does not create the parent it is written into", function()
            ...
            local file
            file, reason = files.open(at("still-nowhere/x.txt"), "a")
            assert.is_nil(file)
            assert.is_true(#reason > 0)
    ...
    describe("streaming reads", function()
        it("treats a non-positive count as one byte through SDL", function()
            ...
            local reader = assert(files.open(at("stream.txt")))
        ...
        it("reports and repositions the file cursor from every origin", function()
            ...
            local reader = assert(files.open(at("seek.bin")))
        ...
        it("allows the EOF position and rejects positions outside the file", function()
            ...
            local reader = assert(files.open(at("bounds.bin")))
        ...
        it("reads directly into a buffer at a caller-selected offset", function()
            ...
            local reader = assert(files.open(at("direct.bin")))
        ...
        it("raises for invalid seeks and closes idempotently", function()
            ...
            local reader = assert(files.open(at("closed.bin")))

            assert.has_error(function()
                reader:seek("middle", 0)
            end, "tecs: File:seek origin must be 'start', 'current', or 'end'")
            assert.has_error(function()
                reader:seek("start", 0.5)
            end, "tecs: File:seek offset must be an integer")
            ...
            assert.is_nil(bytes)
            assert.are.equal("the file is closed", readReason)
            assert.is_nil(size)
            assert.are.equal("the file is closed", sizeReason)
            assert.is_nil(position)
            assert.are.equal("the file is closed", seekReason)
        end)
    end)
    ...
    describe("streaming writes", function()
        ...
        it("uses one shared cursor for update modes", function()
            local path = at("read-write.bin")
            local file = assert(files.open(path, "w+"))

            assert.is_true(file:write("abcdef"))
            assert.are.equal(2, file:seek("start", 2))
            assert.are.equal("cd", file:read(2))
            assert.is_true(file:write("XY"))
            assert.is_true(file:close())
            assert.are.equal("abcdXY", files.read(path))

            file = assert(files.open(path, "a+"))
            assert.are.equal(0, file:tell())
            assert.are.equal("ab", file:read(2))
            assert.are.equal(0, file:seek("start"))
            assert.is_true(file:write("!"))
            assert.is_true(file:close())
            assert.are.equal("abcdXY!", files.read(path))
        end)

        it("reports operations forbidden by the mode", function()
            local path = at("direction.bin")
            assert.is_true(files.write(path, "data"))
            local reader = assert(files.open(path))
            local wrote, writeReason = reader:write("x")
            assert.is_false(wrote)
            assert.are.equal("the file is not open for writing", writeReason)
            assert.is_true(reader:close())

            local writer = assert(files.open(path, "w"))
            local bytes, readReason = writer:read(1)
            assert.is_nil(bytes)
            assert.are.equal("the file is not open for reading", readReason)
            assert.is_true(writer:close())
        end)

        it("rejects an unknown mode", function()
            assert.has_error(function()
                files.open(at("mode.txt"), "unknown")
            end, "tecs: io.files.open mode must be 'r', 'w', 'a', 'r+', 'w+', or 'a+'")
        end)
        ...
        it("keeps seekable writes inside the current destination", function()
            local writer = assert(files.open(at("seek-bounds.bin"), "w"))
            ...
            assert.is_nil(position)
            assert.are.equal("the file is closed", reason)
        end)
    end)
