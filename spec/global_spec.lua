...
describe("the tecs global", function()
    ...
    it("resolves an engine module lazily through the same metatable", function()
        ...
        assert.is_true(rawequal(gfx.Camera3D, require("tecs.gfx.Camera3D")))
        assert.is_true(rawequal(gfx.Camera3D, gfx.Camera3D))
        assert.is_nil(rawget(gfx, "Camera3D"))
        assert.is_true(rawequal(gfx.FlyCamera3D, require("tecs.gfx.FlyCamera3D")))
        assert.is_true(rawequal(gfx.FlyCamera3D, gfx.FlyCamera3D))
        assert.is_nil(rawget(gfx, "FlyCamera3D"))
    end)
