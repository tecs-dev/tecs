...
describe("tecs.io buffers", function()
    it("exposes only the newBuffer constructor", function()
        assert.is_nil(rawget(ioModule, "Buffer"))
    ...
    it("checks ranges, integer arguments, and release", function()
        local buffer = ioModule.newBuffer("abcd")
        assert.has_error(function()
            buffer:getString(5)
        end)
...
describe("tecs.io stream endpoints", function()
    ...
    it("rejects aliased and released memory writers", function()
        local buffer = ioModule.newBuffer("same")
