...
describe("filesystem paths", function()
    ...
    it("normalizes lexically and exposes file-name components", function()
        local path = tecs.io.Path.new("alpha", ".", "beta", "..", "archive.tar.gz")
        local normalized = path:normalize()

        assert.are.equal(tecs.io.Path.new("alpha", "archive.tar.gz"), normalized)
        ...
        assert.are.equal(tecs.io.Path.new("alpha"), normalized:parent())
    ...
    it("resolves absolute and relative paths without requiring them to exist", function()
        ...
        local destination, resolveReason = tecs.io.Path.new("alpha", "file.txt"):resolve()
        ...
        assert.are.equal(tecs.io.Path.new("alpha", "file.txt"), relative)
    end)
    ...
    it("rejects values that cannot be represented as paths", function()
        assert.has_error(function()
            tecs.io.Path.new("bad\0path")
        end)
        assert.has_error(function()
            tecs.io.Path.new("file.txt"):withFileName("two/components")
        end)
    end)
end)
