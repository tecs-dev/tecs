...
describe("filesystem paths", function()
    ...
    it("normalizes lexically and exposes file-name components", function()
        local path = tecs.io.path.newPath("alpha", ".", "beta", "..", "archive.tar.gz")
        local normalized = path:normalize()

        assert.are.equal(tecs.io.path.newPath("alpha", "archive.tar.gz"), normalized)
        ...
        assert.are.equal(tecs.io.path.newPath("alpha"), normalized:parent())
    ...
    it("resolves absolute and relative paths without requiring them to exist", function()
        ...
        local destination, resolveReason = tecs.io.path.newPath("alpha", "file.txt"):resolve()
        ...
        assert.are.equal(tecs.io.path.newPath("alpha", "file.txt"), relative)
    end)
    ...
    it("rejects values that cannot be represented as paths", function()
        assert.has_error(function()
            tecs.io.path.newPath("bad\0path")
        end)
        assert.has_error(function()
            tecs.io.path.newPath("file.txt"):withFileName("two/components")
        end)
    end)
end)
