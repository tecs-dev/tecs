...
describe("render extraction", function()
    ...
    it("reports what it laid out from a known world", function()
        ...
        for _ = 1, 3 do
            world:spawn(Transform(8, 8, 0, 1, 0, 4, 4), Tint(1, 1, 1, 1),
                Renderable())
        end
        ...
        assert.are.equal(3, packet.rewritten,
            "the first frame lays the whole run out")
    ...
    it("marks the bytes it wrote and nothing else", function()
        ...
        assert.are.equal(1, packet.instanceRanges.count,
            "one archetype is one contiguous run")
    ...
    it("marks nothing on a frame where nothing changed", function()
        ...
        assert.are.equal(0, packet.instanceRanges.count,
            "a still frame gives the backend nothing to copy")
    end)
    ...
    it("writes instances into the staging it was handed", function()
        ...
        world:spawn(Transform(12, 34, 0, 1, 0, 4, 4), Tint(0.25, 0.5, 0.75, 1),
            Renderable())
    ...
    it("packs light entities into the packet's own storage", function()
        local world, _, packet = newExtraction()
        world:spawn(Transform(10, 20, 0, 1, 0, 1, 1),
            PointLight(12.0, 30.0, 1.0, 0.5, 0.25, 4.0))
    ...
    it("tells whoever owns the packet that it is filled", function()
        ...
        local seen = 0
        extractor:install(world, packet, function() seen = seen + 1 end)
        world:update(1 / 60)
        world:update(1 / 60)
...
describe("render backend", function()
    ...
    it("draws a packet built by hand, with no world anywhere", function()
        ...
        local centre = screen:getPixel(consume(backend, packet),
            SIZE / 2, SIZE / 2)
    ...
    it("lights a packet from the light array it carries", function()
        ...
        assert.is_true(screen:getPixel(pixels, SIZE / 2, SIZE / 2).r > 180,
            "the packet's light lit its own frame")
        assert.is_true(screen:getPixel(pixels, 2, 2).r < 60,
            "and only as far as its radius")
        backend:destroy()
    end)
    ...
    it("names nothing from the ECS", function()
        local forbidden = {
            "tecs%.ecs", "tecs%.components", "tecs%.internal",
            "tecs%.Extractor",
        }
        for _, module in ipairs({ "Backend", "FramePacket" }) do
            ...
            for _, name in ipairs(forbidden) do
                assert.is_nil(source:match('require%("' .. name .. '[%."]'),
                    ("tecs.%s must not require %s")
                        :format(module, name:gsub("%%", "")))
            end
        end
    end)
