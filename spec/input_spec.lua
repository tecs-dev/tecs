...
local function fakeBackend()
    ...
    local backend = setmetatable({
        ...
        gamepadHasButton = function()
            return true
        end,
        gamepadHasAxis = function()
            return true
        end,
        gamepadHasSensor = function()
            return true
        end,
        gamepadButtonLabel = function()
            return "cross"
        end,
        gamepadPower = function()
            return "onBattery", 42
        end,
        setGamepadSensor = function()
            return true
        end,
        gamepadSensorEnabled = function()
            return true
        end,
        ...
        setLED = function(handle, red, green, blue)
            calls.leds[#calls.leds + 1] = { device = handle.device, red = red, green = green, blue = blue }
            return true
        end,
        ...
        textInputActive = function()
            return calls.textStarted > calls.textStopped
        end,
...
describe("platform.Input", function()
    ...
    it("does not treat key repeats as presses", function()
        ...
        assert.is_false(input:keyPressed("space"), "holding a key must not read as pressing it again")
    end)
    ...
    it("keeps a press that began and ended between fixed steps", function()
        ...
        assert.is_true(input:keyPressed("space"), "the fixed step must see the press it would otherwise miss")
        assert.is_true(input:keyReleased("space"))
    ...
    it("hides input from layers beneath a blocking one", function()
        ...
        assert.is_false(input:keyDown("space"), "gameplay goes quiet without knowing a menu exists")
    ...
    it("accumulates whole scroll notches beside the fractional pair", function()
        ...
        assert.are.equal(1, input.wheelTicksY, "and the event that completed it is one")
    ...
    it("releases everything held when the window loses focus", function()
        ...
        assert.is_true(input:keyReleased("space"), "and the release has to be reported, not merely implied")
        assert.is_false(input:mouseDown("left"))
    end)
    ...
    it("folds the pen axes it names into the pen state", function()
        ...
        assert.are.equal(0.0, input.penPressure, "nothing has been reported yet")
        ...
        assert.are.equal(0.75, input.penPressure, "one axis per event, so the others keep what they held")
...
describe("platform.Input gamepads", function()
    ...
    it("opens what is already attached rather than waiting for an event", function()
    ...
    it("keeps two pads from answering for each other", function()
        ...
        assert.is_true(second:buttonDown("south"), "one pad releasing must not release the other's")
    ...
    it("keeps two pads' axes apart and applies a deadzone", function()
        ...
        assert.are.equal(0.0, first:axis("leftX"), "stick drift is hardware, not gameplay, so it is filtered here")
        assert.is_true(first:axis("leftX", 0.01) > 0.0, "an explicit smaller deadzone still sees it")
    end)

    it("names buttons positionally and reports the printed label apart", function()
    ...
    it("connects and disconnects on the platform's own events", function()
        ...
        assert.are.same({ 21 }, backend.calls.closed, "the handle has to be given back, not merely forgotten")
    end)
    ...
    it("leaves a retained reference to a removed pad safe to call", function()
        ...
        assert.is_false(pad:buttonDown("south"), "a button cannot stay held on a device that is gone")
        assert.is_true(pad:buttonReleased("south"), "and the release is reported, so a held action ends")
    ...
    it("re-reads a pad's description when the platform remaps it", function()
        ...
        backend.gamepadButtonLabel = function()
            return "a"
        end
        input:handleEvent({ kind = "gamepadRemapped", which = 11 })
        assert.are.equal("a", pad:label("south"), "a remap changes what is printed on the button")
    end)
...
describe("platform.Input text", function()
    ...
    it("delivers characters only once a session is started", function()
        ...
        assert.are.same(
            { x = 20, y = 20, width = 400, height = 40, cursor = 8 },
        ...
        assert.are.equal("hi", input.text, "two characters in one frame are both delivered")
    ...
    it("reports what an input method is composing before it commits", function()
        ...
        assert.are.equal("", input.composition, "committing ends the composition, so a field draws it once")
    end)
    ...
    it("stops the session with the layer that started it", function()
        ...
        assert.are.equal("", input.composition, "what was being composed goes with the session")
    end)
